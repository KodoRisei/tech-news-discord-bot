...
| インテリジェントフィルタリング | キーワード + 新鮮度 + ソース信頼性でホットな話題を優先 |
...
キーワードフィルタリングでは、以下の3要素を組み合わせて **ホットな話題** を優先的に選定します。

### スコアリングの仕組み

| 要素 | 重み | 説明 |
|------|------|------|
| **キーワードマッチ** | タイトル: ×3<br>本文: ×1 | 設定キーワードの出現回数でスコア加算 |
| **新鮮度ブースト** | 24時間以内: +5.0<br>1週間以内: +2.0<br>1ヶ月以内: +0.5 | 新しい記事ほど高スコア（ホットな話題を優先） |
| **ソース信頼性** | AWS/Databricks: ×1.3<br>その他: ×1.0 | 公式ソースからの情報を重視 |

この3要素の組み合わせにより、**キーワードに関連しつつ、最近話題になっている記事** が最優先で通知されます。
